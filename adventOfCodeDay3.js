...
function movements(arr){
    ...
    for(let i=0; i < arrLength; i++){
        let direction = arr[i];
        let amount = arr[i].substr(1);
            //amount of movement in specified direction
            switch(direction.charAt(0)){
                case 'R':
                    for(let j = 0; j < amount; j++){
                        coordinates.push({x: ++dirX, y: dirY, amount: ++counter})
                    }
                    break;
                case 'L':
                    for(let j = 0; j < amount; j++){
            ...
            // for(let j = 0; j < amount; j++){
            //     if(direction ==="R")dirX ++;
            //     else if (direction ==="U")dirY ++; 
            //     else if (direction ==="L")dirX --;
            //     else if (direction ==="D")dirY --;
            //     coordinates.push({x: dirX, y: dirY});
            // }
        }
    return coordinates;
}

let wireOneCoordinates = movements(wireOne);
let wireTwoCoordinates = movements(wireTwo);

//calculate where the two wires intersect, and push number of steps into an empty array
let intersections = [];
for (let i = 0; i < wireOneCoordinates.length; i++){
    for (let j = 0; j < wireTwoCoordinates.length; j++){
        if (wireOneCoordinates[i].x === wireTwoCoordinates[j].x && wireOneCoordinates[i].y === wireTwoCoordinates[j].y){
            let x = Math.abs(wireOneCoordinates[i].x);
            let y = Math.abs(wireOneCoordinates[i].y);
...
//calculate the intersection with the shortest distance from the starting point {0, 0}, and push the number of steps into an empty array
let fewestSteps = [];
for (let i = 0; i < wireOneCoordinates.length; i++){
    for (let j = 0; j < wireTwoCoordinates.length; j++){
        if (wireOneCoordinates[i].x === wireTwoCoordinates[j].x && wireOneCoordinates[i].y === wireTwoCoordinates[j].y){
            fewestSteps.push(wireOneCoordinates[i].amount + wireTwoCoordinates[j].amount)
        }
    }
}

console.log(`Fewest amount of steps: ${fewestSteps.sort((a, b) => a - b)[0]}`);